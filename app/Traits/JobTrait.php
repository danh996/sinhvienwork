...

{
...

    {
        ...
        $job->description = nl2br($request->description);
        $job->save();
...

    {
        ...
        return view('job.add_edit_job')->with('countries', $countries)
        ->with('jobTypes', $jobTypes);
    }
...

    {
        ...
        //dd($company->countPostedJobToday());
        if($company->countPostedJobToday() == 3)
        {

            flash(__('Bạn chỉ có thể đăng tối đa 3 tin/ngày'))->error();
        ...
        flash('Thêm việc làm thành công!')->success();
    ...
    public static function countNumJobs($field = 'title', $value = '')
    {
        if (!empty($value)) {
            ...
            if ($field == 'job_type_id') {
                return DB::table('jobs')->where('job_type_id', '=', $value)->where('is_active', '=', 1)
...

    {
        ...
        $row = DB::table('jobs')
        ->where('id', $id)
        ->whereDate('up_top_at', Carbon::now())->first();

        if($row){
            flash('Bạn chỉ có thể đẩy tin 1 lần/ngày')->error();
            return \Redirect::route('posted.jobs');
        }
        $totalActiveJob = Job::count();

        $job->order = $totalActiveJob;
        $job->up_top_at = Carbon::now();

        $job->update();

        flash('Đẩy tin thành công!')->success();
