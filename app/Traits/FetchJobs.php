...
trait FetchJobs
{
    ...
    public function fetchJobs($search = '', $job_titles = array(), $company_ids = array(), $industry_ids = array(), $job_skill_ids = array(), $functional_area_ids = array(), $country_ids = array(), $is_freelance = -1, $career_level_ids = array(), $job_type_ids = array(), $job_shift_ids = array(), $gender_ids = array(), $degree_level_ids = array(), $job_experience_ids = array(), $salary_from = 0, $salary_to = 0, $salary_currency = '', $is_featured = -1, $orderBy = 'id', $limit = 10)
    {
        $asc_desc = 'DESC';
        $query = Job::select($this->fields);
        $query = $this->createQuery($query, $search, $job_titles, $company_ids, $industry_ids, $job_skill_ids, $functional_area_ids, $country_ids, $is_freelance, $career_level_ids, $job_type_ids, $job_shift_ids, $gender_ids, $degree_level_ids, $job_experience_ids, $salary_from, $salary_to, $salary_currency, $is_featured);

        //$query->orderBy('jobs.is_featured', 'DESC');
        $query
        ->orderBy('jobs.order', 'DESC')
        ->orderBy('jobs.up_top_at', 'DESC');
    ...
    public function createQuery($query, $search = '', $job_titles = array(), $company_ids = array(), $industry_ids = array(), $job_skill_ids = array(), $functional_area_ids = array(), $country_ids = array(), $is_freelance = -1, $career_level_ids = array(), $job_type_ids = array(), $job_shift_ids = array(), $gender_ids = array(), $degree_level_ids = array(), $job_experience_ids = array(), $salary_from = 0, $salary_to = 0, $salary_currency = '', $is_featured = -1)
    {
        ...
        if (isset($country_ids[0]) && $country_ids[0] != 0) {
            $query->where('jobs.country_id', $country_ids);
        }
        ...
        if (isset($job_type_ids[0]) && $job_type_ids[0] != 0) {
            $query->where('jobs.job_type_id', $job_type_ids);
        }
    ...
    public function fetchJobsNew($country_ids, $job_type_ids)
    {
        $asc_desc = 'DESC';
        $query = Job::select($this->fields);
        $query = $this->createQueryNew($query, $country_ids, $job_type_ids);

        $query->orderBy('jobs.order', 'DESC')->orderBy('jobs.up_top_at', 'DESC');
        //echo $query->toSql();exit;
        return $query->paginate(20);
    }

        public function createQueryNew($query, $country_ids, $job_type_ids){
