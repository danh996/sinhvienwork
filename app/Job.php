...
class Job extends Model
{
    ...
    public function getTimeCreated(){
        ...
        if($createdAt){
            ...
            $diffSec    = $now->diffInSeconds($createdAt);
            if($diffSec < 60) return $diffSec." giây trước";

            $diffMin    = $now->diffInMinutes($createdAt);
            if($diffMin < 60) return $diffMin." phút trước";

            $diffHour   = $now->diffInHours($createdAt);        
            if($diffHour < 24) return $diffHour ." giờ trước";

            $diffDay   = $now->diffInDays($createdAt);        
    ...
    function showSalary(){
        if($this->salary_from && $this->salary_to){
            return $this->formatMoney($this->salary_from) . ' - ' . $this->formatMoney($this->salary_to). ' đ';
        } elseif($this->salary_from){
            return 'Từ '.$this->formatMoney($this->salary_from);
        } elseif($this->salary_to){
            return 'Đến '.$this->formatMoney($this->salary_to);
        } else{
            return "Liên hệ";
        }
